...
mdData=read.csv('~/Documents/R/MortgageDefaultersDataSample.csv')
summary(mdData)
```
...
```{r}
#Ccreate a dataframe to have a table with the two columns we need that status and outcome

dat <- data.frame(table(mdData$Status,mdData$OUTCOME))

# assign names to the axes, c here is for combining the columns and rbind is for rows

names(dat) <- c("Status", "OUTCOME", "Count")

# plot the frame, data ,x, y and fill is color
ggplot(data=dat, aes(x=Status, y=Count, fill=OUTCOME)) +
...
#GSUB IS FOR REPLACING ANY VALUE
#Does this works
...
dat <- data.frame(table(mdData$First_home,mdData$OUTCOME))

# assign names to the axes, c here is for combining the columns and rbind is for rows

names(dat) <- c("First_home", "OUTCOME", "Count")

# plot the frame, data ,x, y and fill is color
ggplot(data=dat, aes(x=First_home, y=Count, fill=OUTCOME)) +
geom_bar(stat="identity")
...
#the dependent varianle tha we need to predict, y, that is OUTCOME
#the . means take all the feature values. 
# if you wanna prune, or anything else, just put it in method. 
# you will need to make your model better than this simple one. change confidence, prune and entropy and stuff...
...
#nrow(md) means just the rows in md
nr=nrow(md)
...
mdTst = md[-trnIndex,]  #test data with the other row-indices, minus sign here means whats not in train thats in our test
